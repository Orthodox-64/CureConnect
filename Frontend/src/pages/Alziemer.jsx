...
export default function AlzheimerVisionAI() {
    ...
    const generatePDF = () => {
        ...
        try {
            ...
            const filename = `ALZIEMER_Scan_Report_${user?.name?.replace(/\s+/g, '_') || 'Patient'}_${new Date().toLocaleDateString().replace(/\//g, '-')}.pdf`;
            doc.save(filename);
