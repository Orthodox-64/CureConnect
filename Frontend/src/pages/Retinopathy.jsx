...
const formatAnalysisResults = (rawResult) => {
    ...
    const formattedLines = lines.map(line => {
        if (line.toLowerCase().includes('Diabetes')) {
            return `**${line}**`;
        }
...
const analyzeImage = async (imageUrl) => {
  try {
      ...
      const result = await ai.models.generateContent({
          ...
          contents: [
              { role: "user", parts: [{ text: "You are an expert dermatologist specializing in Dermatoscopy  analysis. Analyze the provided retina scan image and determine whether it indicates which type of disease and is there any serious issue. Provide a confidence score (in percentage) for your diagnosis. If retinol or diabetics issue is detected, also mention the suspected type and affected region with a probability score give the whole response only in 5-6 lines and in a in a user Friendly language" }] },
...
export default function RetinolVisionAI() {
    ...
    const generatePDF = () => {
        ...
        try {
            ...
            const filename = `Retinopathy_Scan_Report_${user?.name?.replace(/\s+/g, '_') || 'Patient'}_${new Date().toLocaleDateString().replace(/\//g, '-')}.pdf`;
            doc.save(filename);
